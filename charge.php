...
if (! $user = $DB->get_record("user", array("id" => $data->userid))) {
    message_striperequest_error_to_admin("Not a valid user id", $data);
    redirect($CFG->wwwroot);
}

if (! $course = $DB->get_record("course", array("id" => $data->courseid))) {
    message_striperequest_error_to_admin("Not a valid course id", $data);
    redirect($CFG->wwwroot);
}

if (! $context = context_course::instance($course->id, IGNORE_MISSING)) {
    message_striperequest_error_to_admin("Not a valid context id", $data);
    redirect($CFG->wwwroot);
}
...
if (! $plugininstance = $DB->get_record("enrol", array("id" => $data->instanceid, "status" => 0))) {
    message_striperequest_error_to_admin("Not a valid instance id", $data);
    redirect($CFG->wwwroot);
}
...
if ($data->courseid != $plugininstance->courseid) {
    message_striperequest_error_to_admin("Course Id does not match to the course settings, received: ".$data->courseid, $data);
    redirect($CFG->wwwroot);
}

$plugin = enrol_get_plugin('striperequest');
...
try {
    ...
    if ($data->coupon_id && $data->coupon_id != 0) {
        ...
        if (!$coupon->valid) {
            redirect($CFG->wwwroot.'/enrol/index.php?id='.$data->courseid, get_string("invalidcouponcodevalue",
                "enrol_striperequest", $data->coupon_id));
        } else {
            $iscoupon = true;
    ...
    $checkcustomer = $DB->get_records('enrol_striperequest',
    ...
    if (!$checkcustomer) {
        ...
        "description" => get_string('charge_description1', 'enrol_striperequest'));
        if ($iscoupon) {
    ...
    $DB->insert_record("enrol_striperequest", $data);
    ...
    if ($users = get_users_by_capability($context, 'moodle/course:update', 'u.*', 'u.id ASC',
		...
		// Integration with enrol_request - Change status of users that were successfully enrolled -- Melvyn Gomez OpenRanger SA de CV 03/18/2020
		enrol_request_change_status_enrolled($course, $user);
 ...
 * @param stdClass $data
 */
function message_striperequest_error_to_admin($subject, $data) {
    $admin = get_admin();
    $site = get_site();
